...
use git_reabsorb::models::{
    CommitDescription, HunkId, PlannedChange, PlannedCommit, PlannedCommitId,
};
...
"#,
    ...
    let diff = repo.git.get_working_tree_diff().unwrap();
    let hunks = git_reabsorb::patch::parse(&diff, std::slice::from_ref(&original_head), 0)
        .unwrap()
        .hunks;
...
fn test_diff_trees_produces_parseable_hunks_for_new_files() {
    ...
    let hunks = parse(&diff_output, std::slice::from_ref(&head), 0)
        .unwrap()
        .hunks;
...
fn test_squash_with_file_deletions() {
    ...
    for (i, hunk) in hunks.iter().enumerate() {
        eprintln!(
            "  Hunk {}: {} (old_count={}, new_count={}, lines={})",
            i,
    ...
    let delete1_hunks: Vec<_> = hunks
        .iter()
        .filter(|h| h.file_path == Path::new("delete1.txt"))
        .collect();
    let delete2_hunks: Vec<_> = hunks
        .iter()
        .filter(|h| h.file_path == Path::new("delete2.txt"))
        .collect();
    let new_hunks: Vec<_> = hunks
        .iter()
        .filter(|h| h.file_path == Path::new("new.txt"))
        .collect();

    assert!(
        !delete1_hunks.is_empty(),
        "Should have hunk for delete1.txt"
    );
    assert!(
        !delete2_hunks.is_empty(),
        "Should have hunk for delete2.txt"
    );
    ...
    assert_eq!(
        deletion_hunks.len(),
        2,
        "Should have 2 deletion hunks (delete1.txt and delete2.txt)"
    );
    ...
    eprintln!(
        "delete1.txt content in HEAD: {:?} (len={}, ends with newline={})",
    ...
    assert!(
        !repo.path.join("delete1.txt").exists(),
        "delete1.txt should be deleted"
    );
    assert!(
        !repo.path.join("delete2.txt").exists(),
        "delete2.txt should be deleted"
    );
    assert!(repo.path.join("new.txt").exists(), "new.txt should exist");
    assert!(
        repo.path.join("keep.txt").exists(),
